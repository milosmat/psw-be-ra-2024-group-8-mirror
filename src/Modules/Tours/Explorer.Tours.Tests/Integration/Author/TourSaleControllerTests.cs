...
namespace Explorer.Tours.Tests.Integration.Author
{
    ...
    public class TourSaleControllerTests : BaseToursIntegrationTest
    {
        ...
        public void Updates_TourSale()
        {
            ...
            var existingTourSale = dbContext.TourSales.AsNoTracking().FirstOrDefault(ts => ts.Id == 1);
            if (existingTourSale == null)
            {
                existingTourSale = new TourSale(new List<int> { 1, 2, 3 }, DateTime.Now.ToUniversalTime(), DateTime.Now.AddDays(7).ToUniversalTime(), 10.0, true, 123);
            ...
            {
                Id = (int)existingTourSale.Id,
                Tours = new List<int> { 4, 5, 6 },
                StartDate = existingTourSale.StartDate.ToUniversalTime(),
                EndDate = existingTourSale.EndDate.AddDays(5).ToUniversalTime(),
                Discount = 20.0,
                Active = false,
